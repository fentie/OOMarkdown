...
class Markdown
{
    ...
    # hard-coded:
    /**
     * Takes the passed in HTML and stores block elements in a hash map
     * @param string $text
     * @return string
     */
    ...
    /**
     * Backtick quotes are used for <code></code> spans. You can use multiple 
     * backticks as the delimiters if you want to include literal backticks in 
     * the code span.
     * 
     * @param string $text
     * @return string
    ...
    /**
     * Do transforms on all <blockquote> tags
     * 
     * @param string $text
     * @return string
     */
    ...
    /**
     * Transform a single blockquote element
     * 
     * @param array $matches
     * @return string 
     */
    ...
    /**
     * Encodes each character of an email address as either deximal or hex in 
     * order to fool spam bots. Based on a filter by Matthew Wickline, poasted 
     * to the BBEdit-Talk mailing list
     * 
     * @see http://tinyurl.com/yu7ue
    ...
    {   
        $depth = 6;
        $nested_tags = implode('|', str_repeat('(?:<[a-z/!$](?:[^<>]', $depth) . str_repeat(')*>)', $depth));
        $pattern = "/(?s: <! ( -- .*? -- \s* )+ > )(?s: <\? .*? \?> )$nested_tags/ix";
//TODO: this doesn't quite work yet. revisit this if it's even needed
        $pos = 0;
        $tokens = array();
        while ($matches = preg_match("/($pattern)/g", $str)) {
            $wholeTag = $match[1];
            $sec_start = pos($str);
            $tag_start = $sec_start - strlen($wholeTag);
            if ($pos < $tag_start) {
                $tokens[] = array('text' => substr($str, $pos, $tag_start - $pos));
            }
            $tokens[] = array('tag' => $wholeTag);
            $pos = pos($str);
        }
        $len = strlen($str);
        if ($pos < $len) {
            $tokens[] = array('text' => substr($str, $pos, $len - $pos));
        }
        return $tokens;
    }
...
protected function escapeSpecialChars($text)
{
    ...
    //rebuild the text from the tokens
    $text = '';
    ...
    foreach ($tokens as $token) {
        if (key($token) === 'tag') {
            ...
            $token->[1] =~  s! \* !$g_escape_table{'*'}!gx;
            $token->[1] =~  s! _  !$g_escape_table{'_'}!gx;
            $text .= $token->[1];
        } else {
            my $t = $cur_token->[1];
